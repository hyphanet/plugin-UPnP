...
*       - Problem : If the host does not start with a '/', the device could refuse the control
*         action
*       - Error : it is not an error, but adding the '/' when missing allows the integration with
*         the Intel devices
*   09/02/03
...
*       - Error : the presence of a base url is not mandatory, the API code makes the assumption
*         that control and event subscription urls are relative.
*         If the baseUrl is not present, the request host and port should be extracted from the
...
*       - The xml nodes controlUrl and eventSubUrl can contain absolut urls, but these absolut urls
*         may have different ports than the base url! (so seen on my SMC 7004ABR Barricade Router,
*         where xml files are requested from port 80, but soap requests are made on port 5440).
*         Therefore whenever a request is made, the port specified by the controlUrl or eventSubUrl
*         node should be used, else no response will be returned
*         (oddly, there was a response returned even on port 80, but with empty body tags. but the
*         correct response finally came from port 5440).
