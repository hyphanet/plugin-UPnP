...
public class UPnP extends ControlPoint
    ...
    public DetectedIP[] getAddress() {
        ...
        if (isDisabled) {
            ...
            Logger.normal(
                this,
                "No UP&P device found, detection of the external ip address using the plugin has failed");
    ...
    public void deviceAdded(Device dev) {
        ...
        if ( !ROUTER_DEVICE.equals(dev.getDeviceType()) || !dev.isRootDevice()) {
            ...
            Logger.error(
                this,
                "We got a second IGD on the network! the plugin doesn't handle that: let's disable it.");
            System.err.println(
                "The UP&P plugin has found more than one IGD on the network, as a result it will be disabled");
            isDisabled = true;
