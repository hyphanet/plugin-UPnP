...
*       - Problem : The device does not accepts request for services when control or subscription
                    urls are absolute
*       - Error : device methods, when requests are received, search for services that have a
                  controlUrl (or eventSubUrl) equal to the request URI
...
*       - problem: not able to retrieve service information when URLBase is missing and SCPDURL is
*         relative
*       - fix: modify to retrieve host information from Header's Location (required) field and
*              update theBaseURL tag in the xml so subsequent information retrieval can be done
*              (Steven Yen, 8.27.2003)
*       - note: 1. in the case that Header's Location field combine with SCPDURL is not able to
*                  retrieve proper information, updating BaseURL would not hurt, since exception
*                  will be thrown with or without update.
*               2. this problem was discovered when using PC running MS win XP with ICS enabled
*                  (gateway). It seems that  root device xml file does not have BaseURL and
*                  SCPDURL are all relative.
*               3. UPnP device architecture states that BaseURL is optional and SCPDURL may be
*                  relative as specified by UPnP vendor, so MS does not seem to violate the rule.
*   10/22/03
...
*   07/31/04
*       - Changed notify() to remove the expired subscribers and not to remove the invalid response
*         subscribers for NMPR.
*   10/29/04
