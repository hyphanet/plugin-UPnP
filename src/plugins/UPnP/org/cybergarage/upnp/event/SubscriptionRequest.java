...
*       - Problem : If the EventSubURL does not start with a '/', the device could refuse event
*         subscription
...
*         that control and event subscription urls are relative. If the baseUrl is not present, the
*         request host and port should be extracted from the control/subscription url
...
*       - Changed setServie() to get the host address from the SSDP Location field when the URLBase
*         is null.
*   12/06/04
