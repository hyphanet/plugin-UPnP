 ...
 * META-INF/services/plugins.JabberLinker.org.xmlpull.v1.XmlPullParserFactory resource that should
 * contain a comma separated list of class names of factories or parsers to try (in order from
...
public class XmlPullParserFactory {
    ...
    public XmlPullParser newPullParser() throws XmlPullParserException {
        ...
        if (parserClasses.size() == 0) {
            throw new XmlPullParserException("No valid parser classes found in " +
                    classNamesLocation);
        }
    ...
    public XmlSerializer newSerializer() throws XmlPullParserException {
        if (serializerClasses == null) {
            throw new XmlPullParserException("Factory initialization incomplete - has not tried " +
                    classNamesLocation);
        }

        if (serializerClasses.size() == 0) {
            throw new XmlPullParserException("No valid serializer classes found in " +
                    classNamesLocation);
        }
    ...
    public static XmlPullParserFactory newInstance(String classNames, Class context)
        ...
        if ((classNames == null) || (classNames.length() == 0) || "DEFAULT".equals(classNames)) {
            try {
                ...
                if (is == null) {
                    throw new XmlPullParserException("resource not found: " + RESOURCE_NAME +
